...
def test_deploy_help_lists_only_supported_targets() -> None:
    """The LangSmith / LangGraph deploy target was removed in Phase 7."""
    runner = CliRunner()
    result = runner.invoke(cli, ["deploy", "-h"])

    assert result.exit_code == 0
    assert "ec2" in result.output
    assert "langsmith" not in result.output.lower()
